...
'''
notes
1. uses data that exclude test
2. fully augments data, though data is highly skewed to a ribsome 
...
data1_keys_train = data1_keys[2::3] + data1_keys[1::3] + [x for x in data1 if  len(data1[x][0]) > 500]
data1_keys_val = data1_keys[0::3]
...
# tf Graph input
x = tf.placeholder(tf.float32,[batch_size,15,None,1])
# pairwise distances need to define
resi_map0 = tf.placeholder(tf.float32,[batch_size,None,None,3])
# some values are na, these will be excluded from loss
above_zero = tf.placeholder(tf.float32,[batch_size,None,None,3])
...
# ss_2d
ss_2d = tf.placeholder(tf.float32,[batch_size,None,None])
...
for epoch in range(training_epochs):
    ...
    for size in sorted(data3_x.keys()):
        ...
        if size >=400:
            ...
            num = 8
        for batch in range(0,len(shuffle),100*num):
