...
##data1 = np.load('../../data_test_nan.npy').item()
##data2 = np.load('../../data_test_ss.npy').item()
##data3 = np.load('../../data_test_extra.npy').item()
##data4 = np.load('../../data_test_MSA.npy').item()
...
for epoch in range(training_epochs):
    avg_cost = 0.
    val_cost = 0.
    total_batch = len(data2_x)#int(mnist.train.num_examples/batch_size)
    ...
    for i in range(len(data2_x)):
        if i < train_n:
            ...
            accuracy(np.argmax(pred,3)[0],np.argmax(batch_y,3)[0])
        else:
            batch_x, batch_y = np.array([[data2_x[i],],]),np.array([data2_y[i],])
            batch_y_nan,batch_y_ss = np.array([data2_y_nan[i]]),np.array([data2_y_ss[i]])
            batch_x = np.swapaxes(np.swapaxes(batch_x,1,3),1,2)
