...
def is_rna(str):
    score = 0
    for i in ['A', 'C', 'E', 'D', 'G', 'F', 'I', 'H', 'K', 'M', 'L', 'N', 'Q', 'P', 'S', 'R', 'T', 'W', 'V', 'Y']:
        if i in list(set(str)):
            score += 1
    if score >=5:
        return False
    else:
        ...
        for k in temp[pdb]:
            ...
            for j in range(1,len(html.split('>'))):
                if pdb.upper()+':'+chain.upper() == html.split('>')[j].split('|')[0].upper():
                    if is_rna(clean_string(html.split('>')[j])):
                        all_dict2[(pdb+'_'+chain).lower()] =  clean_string(html.split('>')[j])
                        got = False
        if got is True:
