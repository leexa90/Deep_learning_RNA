...
def make_array2(str):
    ...
    return temp
random.seed(0)
data1_keys = [x for x in data1 if  len(data1[x][0]) <= 1000] # the short ones get val-train split
random.shuffle(data1_keys)
data1_keys_train = data1_keys[0::3] + data1_keys[1::3] #+ [x for x in data1 if  len(data1[x][0]) > 00]
    ...
    if len(data1[i][0]) >= 35:
        if i in data2.keys():
            try:
                ...
                data_train[i] = [tempF, temp1[0],temp1[1],temp_resi_map,d,temp2[1],temp2[0]]
...
##    '4_wc1d': tf.Variable(tf.random_normal([1, 1, num3*4, 16])),
    ...
    '5_wc1bb': tf.Variable(tf.random_normal([4, 4, num5, num5])),
    ...
    '6_wc1bb': tf.Variable(tf.random_normal([4, 4, num6, num6])),
    ...
    '7_wc1bb': tf.Variable(tf.random_normal([4, 4, num7, num7])),
