            ...
            for j in range(i+1,answer.shape[1]):
                if np.sum(answer[j,:]) != 0  :
                    if answer[i,j] == mat_model[i,j]:
                        ...
                        score[answer[i,j]] += [0,]
    return np.mean((map(lambda x : int(x*100),[np.mean(score[0]),np.mean(score[1]),np.mean(score[2])]))),(map(lambda x : int(x*100),[np.mean(score[0]),np.mean(score[1]),np.mean(score[2])]))

data = np.load('val_result.npy').item()
fig ,ax = plt.subplots(1,2)
for i in data:
    ax[0].imshow(data[i][0])
    ax[1].imshow(np.argmax(data[i][1],2))
    acc0,acc = accuracy(data[i][0],np.argmax(data[i][1],2))
    ax[1].set_title(i)
    ax[1].set_xlabel(acc0)
    ax[0].set_xlabel(acc)
    plt.savefig('results/result'+i+'.png')
die
...
for epoch in range(training_epochs):
    ...
    save_path = saver.save(sess,'model300_reweigh_loss_restore.ckpt')
