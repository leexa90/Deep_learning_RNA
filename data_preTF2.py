...
not_zero = tf.cast(not_zero,dtype=tf.float32)
...
out2 = tf.multiply(out,not_zero,name='out')
#out = tf.reshape(out,shape=(-1,100,100))

# Define loss and optimizer
cost = tf.reduce_mean([out2,resi_map])
    ...
    for epoch in range(training_epochs):
        ...
        if epoch % display_step == 0:
            print ("Epoch:", '%04d' % (epoch+1), "cost=", 
                "{:.9f}".format(avg_cost))
        pred = sess.run( out, feed_dict={x: batch_x,resi_map: batch_y})
